...
export const handleLogout = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      // Even if response is not 'ok', if backend successfully cleared the cookie
      // we might still want to proceed with client-side logout.
      // For now, treat non-2xx as an error.
...
export const checkAuthStatus = async () => {
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/me`, {
      credentials: 'include', // Important: Ensures cookies (like session cookie) are sent with the request
    });
    if (response.ok) {
      const user: UserProfile = await response.json();
      // Backend /me endpoint returns null or empty object if not logged in, or user object
      if (user && user.id) {
