...
import { AuthRequest, CreateJwtUserDto, GitHubProfileDto, GitHubTokenDto, GoogleProfileDto, GoogleTokenDto, LoginDto, RegisterDto, VerifyEmailDto, ForgotPasswordDto, ResetPasswordDto, AuthResponseDto, LoginResponseDto } from './types';
...
export async function authRouter(fastify: FastifyInstance) {
  ...
  fastify.get('/google', fastifyPassport.authenticate('google', { scope: ['profile', 'email'], state: (req: FastifyRequest) => JSON.stringify({ cli_port: req.query?.cli_port, csrf_token: '123' }) }));
  ...
  async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      if (request.query?.state) {
        try {
          const parsedState = JSON.parse(decodeURIComponent(request.query.state as string));
          cliPort = parsedState.cli_port;
        } catch (e) {
          request.log.warn('Failed to parse state parameter for cli_port:', e);
        }
      }
  ...
  fastify.get('/github', fastifyPassport.authenticate('github', { scope: ['user:email'], state: (req: FastifyRequest) => JSON.stringify({ cli_port: req.query?.cli_port, csrf_token: '123' }) }));
  ...
  async (request: FastifyRequest, reply: FastifyReply) => {
    try {
      ...
      request.log.error('GitHub OAuth error:', error);
      reply.redirect(`${configService.get('FRONTEND_URL')}/login?error=OAuth%20Login%20Failed`);
    }
  });
