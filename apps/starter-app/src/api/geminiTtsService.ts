...
import { TtsRequestDto } from '../types/tts';
  ...
  generateSpeech: async (data: TtsRequestDto): Promise<Blob> => {
    ...
    try {
      const response = await axios.post<Blob>(
        `${API_BASE_URL}/google-tts/generate`,
        data,
        {
          headers: {
      ...
      if (axios.isAxiosError(error)) {
        ...
        if (error.response?.data instanceof Blob) {
          ...
          try {
            ...
            throw new Error(
              errorJson.message || errorJson.error || 'An unknown API error occurred (parsed from blob)',
            );
          } catch {
            // If it's a blob but not JSON, just return the text
            throw new Error(
              errorText || 'An unknown API error occurred (non-JSON blob response)',
            );
          }
        } else {
