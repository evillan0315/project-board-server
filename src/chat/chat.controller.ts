...
export class ChatController {
  ...
  @ApiQuery({ name: 'userId', type: 'string', description: 'The UUID of the user to fetch conversations for.' })
  @ApiResponse({
    status: HttpStatus.OK,
    description: 'Successfully retrieved the list of conversations.',
    isArray: true,
  })
  async getConversations(@Query() query: GetConversationsDto): Promise<Conversation[]> {
