...
export class GeminiResponseService {
  ...
  onModuleInit() {
    // Optionally, you could log a warning or take action if GeminiResponseModule is disabled on startup
    if (!this.moduleControlService.isModuleEnabled('GeminiResponseModule')) {
      this.logger.warn(
        'GeminiResponseModule is currently disabled via ModuleControlService. GeminiResponse operations will be restricted.',
      );
    }
  }
  private ensureFileModuleEnabled(): void {
    if (!this.moduleControlService.isModuleEnabled('GeminiResponseModule')) {
      throw new ForbiddenException(
        'GeminiResponse module is currently disabled. Cannot perform GeminiResponse operations.',
      );
    }
  }
  ...
  findAll() {
    this.ensureFileModuleEnabled();
    return this.prisma.geminiResponse.findMany();
  }
  ...
  update(id: string, data: UpdateGeminiResponseDto) {
    this.ensureFileModuleEnabled();
    return this.prisma.geminiResponse.update({
      where: { id },
      data,
    });
  }
