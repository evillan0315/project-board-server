...
export class GitController {
  ...
  async stageFiles(
    @Body() dto: GitFilesOperationDto,
  ): Promise<{ message: string }> {
    const message = await this.gitService.stageFiles(
      dto.filePaths,
      dto.projectRoot,
    );
    return { message };
  ...
  async unstageFiles(
    ...
    const message = await this.gitService.unstageFiles(
      dto.filePaths,
      dto.projectRoot,
    );
    return { message };
  ...
  @ApiResponse({
    status: 200,
    description: 'Staged changes reset successfully',
  })
  async resetStagedChanges(
    @Body() dto: GitResetStageDto,
  ): Promise<{ message: string }> {
    if (dto.filePath) {
      const message = await this.gitService.unstageFiles(
        [dto.filePath],
  ...
  @ApiOperation({
    summary:
      'Perform a hard reset to a specific commit or HEAD. WARNING: This discards all uncommitted changes.',
  })
  @ApiResponse({
    status: 200,
    description: 'Hard reset performed successfully',
  })
  async resetHard(@Body() dto: GitResetHardDto): Promise<{ message: string }> {
    const message = await this.gitService.resetHard(
      dto.commitHash,
      dto.projectRoot,
    );
    return { message };
  ...
  async commit(@Body() dto: CommitDto): Promise<CommitResponseDto> {
    const commitMessage = await this.gitService.commit(
      dto.message,
      dto.projectRoot,
    );
  ...
  async revertCommit(
    @Body() dto: RevertCommitDto,
  ): Promise<{ message: string }> {
    const message = await this.gitService.revertCommit(
      dto.commitHash,
      dto.projectRoot,
    );
    return { message };
  ...
  @ApiOperation({
    summary: 'Discard changes in a specific file in the working directory',
  })
  @ApiResponse({ status: 200, description: 'File changes undone successfully' })
  async undoFileChanges(
    @Body() dto: GitFileOperationDto,
  ): Promise<{ message: string }> {
    const message = await this.gitService.undoFileChanges(
      dto.filePath,
      dto.projectRoot,
    );
    return { message };
  ...
  async createBranch(
    ...
    const message = await this.gitService.createBranch(
      dto.newBranchName,
      dto.projectRoot,
    );
    return { message };
  ...
  async checkoutBranch(
    ...
    const message = await this.gitService.checkoutBranch(
      dto.branchName,
      dto.remote || false,
      dto.projectRoot,
    );
  ...
  async deleteBranch(
    ...
    const message = await this.gitService.deleteBranch(
      dto.branchName,
      dto.force || false,
      dto.projectRoot,
    );
  ...
  async createSnapshot(
    ...
    const message = await this.gitService.createSnapshot(
      dto.snapshotName,
      dto.message,
      dto.projectRoot,
    );
  ...
  @ApiOperation({
    summary: 'Restore the repository to a specific snapshot (checkout tag)',
  })
  @ApiResponse({ status: 200, description: 'Snapshot restored successfully' })
  async restoreSnapshot(
    @Body() dto: RestoreSnapshotDto,
  ): Promise<{ message: string }> {
    const message = await this.gitService.restoreSnapshot(
      dto.snapshotName,
      dto.projectRoot,
    );
    return { message };
  ...
  async deleteSnapshot(
    ...
    const message = await this.gitService.deleteSnapshot(
      snapshotName,
      projectRoot,
    );
    return { message };
