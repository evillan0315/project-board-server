...
export class ProposedFileChangeService {
  ...
  onModuleInit() {
    ...
    if (!this.moduleControlService.isModuleEnabled('ProposedFileChangeModule')) {
      this.logger.warn(
  ...
  findOne(id: string) {
    ...
    return this.prisma.proposedFileChange.findUnique(
  ...
  private buildWhereFromQuery(query: PaginationProposedFileChangeQueryDto): Prisma.ProposedFileChangeWhereInput {
