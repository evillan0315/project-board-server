...
export class ResumeController {
  ...
  @ApiOperation({ summary: 'Extract plain text content from a PDF or DOCX resume file' })
  @ApiResponse({ status: 200, description: 'Text successfully extracted from the file.', type: String })
  @ApiResponse({ status: 400, description: 'No file provided, unsupported file type, or file too large/corrupted.' })
  @ApiResponse({ status: 500, description: 'Internal server error during file parsing.' })
      ...
      storage: diskStorage({
        destination: (req: Request, file, cb) => {
          ...
          if (!user) {
            ...
            return cb(new BadRequestException('User ID not available for file upload.'), '');
          }
  ...
  @ApiOperation({ summary: 'Parse a resume file/text and optimize it against a job description using Google Gemini' })
  @ApiResponse({ status: 200, description: 'Resume optimization results returned successfully.', type: OptimizationResultDto })
  @ApiResponse({ status: 400, description: 'Invalid input (missing file/text/job description, unsupported file type, parsing error).' })
  @ApiResponse({ status: 500, description: 'Internal server error during processing or Gemini API call.' })
  async optimizeResumeFromFile(
    ...
    if (resumeFile && !user) {
      ...
      throw new BadRequestException('Authentication required for file uploads.');
    }

    if (!jobDescription || jobDescription.trim() === '') {
      throw new BadRequestException('Job description is required for resume optimization.');
    }

    let resumeData: string;
    if (resumeFile) {
    console.log(resumeFile, 'resumeFile');
      resumeData = await this.resumeParserService.extractTextFromFile(resumeFile);
    } else if (resumeContent && resumeContent.trim() !== '') {
      resumeData = resumeContent.trim();
    } else {
      throw new BadRequestException('Either a resume file or plain text resume content is required for optimization.');
    }
    ...
    const result = await this.googleGeminiFileService.optimizeResume(optimizeDto);
  ...
  @ApiOperation({ summary: 'Generate a new resume based on a detailed prompt using Google Gemini' })
  @ApiBody({ type: GenerateResumeDto })
  @ApiResponse({ status: 200, description: 'Resume generated successfully.', type: String })
  @ApiResponse({ status: 400, description: 'Invalid input or missing prompt.' })
  @ApiResponse({ status: 500, description: 'Internal server error during Gemini API call.' })
  async generateResume(
    @Body() generateResumeDto: GenerateResumeDto,
  ): Promise<string> {
    if (!generateResumeDto.prompt || generateResumeDto.prompt.trim() === '') {
      throw new BadRequestException('Prompt is required for resume generation.');
    }
    return this.googleGeminiFileService.generateResume(generateResumeDto);
  }

  @Post('enhance-resume')
  @ApiOperation({ summary: 'Enhance an existing resume or a specific section using Google Gemini' })
  @ApiBody({ type: EnhanceResumeDto })
  @ApiResponse({ status: 200, description: 'Resume enhanced successfully.', type: String })
  @ApiResponse({ status: 400, description: 'Invalid input or missing resume content.' })
  @ApiResponse({ status: 500, description: 'Internal server error during Gemini API call.' })
  async enhanceResume(
    @Body() enhanceResumeDto: EnhanceResumeDto,
  ): Promise<string> {
    if (!enhanceResumeDto.resumeContent || enhanceResumeDto.resumeContent.trim() === '') {
      throw new BadRequestException('Resume content is required for enhancement.');
    }
